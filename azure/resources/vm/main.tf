...
resource "azurerm_storage_account" "vm-sa" {
  ...
  name                     = format("bootdiag-%s", lower(random_id.vm-sa.hex))
...
resource "azurerm_virtual_machine" "vm-linux" {
  ...
  name                          = format("%s-%s", var.vm_hostname , count.index )
  location                      = var.rg_location
  resource_group_name           = var.rg_name
  ...
  storage_os_disk {
    name              = format("osdisk-%s-%s", var.vm_hostname, count.index)
  ...
  os_profile {
    computer_name  = format("%s%d", var.vm_hostname, count.index)
...
resource "azurerm_virtual_machine" "vm-linux-with-datadisk" {
  ...
  name                          = format("%s-%d", var.vm_hostname,count.index)
  location                      = var.rg_location
  resource_group_name           = var.rg_name
  availability_set_id           = azurerm_availability_set.vm.id
  vm_size                       = var.vm_size
  ...
  storage_os_disk {
    name              = format("osdisk-%s-%d", var.vm_hostname, count.index)
    create_option     = "FromImage"
    caching           = "ReadWrite"
    managed_disk_type = var.storage_account_type
  }
...
resource "azurerm_availability_set" "vm" {
  name                         = format("%s-avset", var.vm_hostname)
  location                     = var.rg_location
  resource_group_name          = var.rg_name
...
resource "azurerm_public_ip" "vm" {
  ...
  name                         = format("%s-%d-publicIP", var.vm_hostname, count.index)
  location                     = var.rg_location
  resource_group_name          = var.rg_name
...
resource "azurerm_network_interface" "vm" {
  ...
  name                      = format("nic-%s-%d", var.vm_hostname, count.index)
  location                  = var.rg_location
  resource_group_name       = var.rg_name
  network_security_group_id = var.nsg_id

  ip_configuration {
    name                          = format("ipconfig%d", count.index)
